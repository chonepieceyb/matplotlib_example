...
styles = get_style_sheet_upon_base("picture_type_3-measure.mplstyle")

labels_univmon = ['4', '8', '12', '16']
labels_SS = ['8', '12', '16', '20']
...
fig_config = {
    'xlabel_1' : 'number of hash function',   #x轴标签名
    'xlabel_2' : ' size of heap',
...
def draw(): 
    with plt.style.context(styles): 
        ...
        x_tick, _, right_edge = cal_bar_xticks(fig_config["bar_edge"] + width/2, 
            len(labels_univmon), width , interval=fig_config["bar_interval"])
        x = np.array(x_tick)
        ...
        ax1.set_xticks(x_label_tick, labels_univmon, **fig_config["xtick_label_conf"])
        ax1.set_title("Vector of Bloom Filter")
        ax1.set_xlabel(fig_config['xlabel_1'])
                       
        ax2.set_xlim(0, right_edge + fig_config["bar_edge"])
        ax2.set_xticks(x_label_tick, labels_SS, **fig_config["xtick_label_conf"])
        ...
        fig.tight_layout() 
        save_figure('picture_type_3-measure')
        plt.show()
